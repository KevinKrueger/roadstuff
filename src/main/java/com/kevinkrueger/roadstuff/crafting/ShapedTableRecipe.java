...
import static net.minecraft.item.crafting.ShapedRecipe.*;
...
public class ShapedTableRecipe implements ISpecialRecipe, ITableRecipe {
    ...
    public static class Serializer extends ForgeRegistryEntry<IRecipeSerializer<?>> implements IRecipeSerializer<ShapedTableRecipe> {
        ...
        public ShapedTableRecipe read(ResourceLocation recipeId, JsonObject json) {
            Map<String, Ingredient> map = com.kevinkrueger.roadstuff.crafting.ShapedRecipe.deserializeKey(JSONUtils.getJsonObject(json, "key"));
            String[] pattern = com.kevinkrueger.roadstuff.crafting.ShapedRecipe.shrink(ShapedTableRecipe.patternFromJson(JSONUtils.getJsonArray(json, "pattern")));
            int width = pattern[0].length();
            int height = pattern.length;
            NonNullList<Ingredient> inputs = com.kevinkrueger.roadstuff.crafting.ShapedRecipe.deserializeIngredients(pattern, map, width, height);
